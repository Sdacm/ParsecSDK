...
int32_t main(int32_t argc, char **argv)
{
	...
	while (!context.done) {
		// TODO:
		// Criar variável int i = 0 fora do while
		// incrementar o valor do i dentro do for
		// if i > 50000 then i = 0
		// calcular distância entre o cursor do host e a pinned position
		// if (i == distancia * 1000) { *alert* }
		...
		for (SDL_Event msg; SDL_PollEvent(&msg);) {
			...
			switch (msg.type) {
				...
				case SDL_KEYDOWN:
					//printf("\a");
					printf("i = %d\n", i);
					//SDL_ShowSimpleMessageBox("LHALH");
				case SDL_KEYUP:
				...
				case SDL_MOUSEMOTION:
					...
					posicaoX = pmsg.mouseMotion.x;
					posicaoY = pmsg.mouseMotion.y;
					d = euclideanDistance(pinned.x, pinned.y, posicaoX, posicaoY);
					if (d < 100) {
						printf("Dentro (%d)\n", j);
						printf("\a");
						j++;
				...
				case SDL_MOUSEBUTTONDOWN:
					//printf("Posicao X: %d\nPosicao Y: %d\n", pmsg.mouseMotion.x, pmsg.mouseMotion.y);
					j = 0;
					pinned.x = posicaoX;
					pinned.y = posicaoY;
